...
class Tide(object):
    ...
    def _prepare(constituents, t0, t=None, radians=True):
        ...
        t0 -- time at which to evaluate speed and equilibrium argument for each
              constituent
        ...
        """
        # The equilibrium argument is constant and taken at the beginning of
        # the time series (t0).  The speed of the equilibrium argument changes
        # very slowly, so again we take it to be constant over any length of
        # data. The node factors change more rapidly.
        ...
        u = [np.mod(np.array([c.u(a_i) for c in constituents])[:, np.newaxis],
                    360.0) for a_i in a]
        f = [np.mod(np.array([c.f(a_i) for c in constituents])[:, np.newaxis],
                    360.0) for a_i in a]
    ...
    def form_number(self):
        ...
        k1, o1, m2, s2 = (
            ...
            for c in [constituent._K1, constituent._O1, constituent._M2,
                      constituent._S2]
        )
        return (k1 + o1) / (m2 + s2)
    ...
    def extrema(self, t0, t1=None, partition=2400.0):
        ...
        t1 -- optional time before which extrema are sought (if not given, the
              generator is infinite)
        partition -- number of hours for which we consider the node factors to
                     be constant (default: 2400.0)
        """
        if t1:
            ...
            for start, end in izip(*partitions):
                ...
                # These derivatives don't include the time dependence of u or
                # f, but these change slowly.
    ...
    def _partition(hours, partition=3600.0):
        ...
        return [hours[np.floor(np.divide(relative, partition)) == i]
